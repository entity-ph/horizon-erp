import { ICreatePackage, ICreatePackageAccommodation, ICreatePackageAirfare, IUpdatePackage, IUpdatePackageAccommodation, IUpdatePackageAirfare, IUpdatePackageStatus } from "@/interfaces/package.interface";
...
export async function updatePackageStatus({ id, status }: IUpdatePackageStatus) {
	try {
		const response = await api.put(`/api/v1/packages/${id}/status`, { status });
		return response.data;
	} catch (error) {
		let message;
		if (error instanceof AxiosError) {
